...
from tools import os_walk, CrossEntropyLabelSmooth, SupConLoss, TripletLoss_WRT, MSEL, MSEL_Feat, MSEL_Cos, KLDivergenceLoss_V2
...
class BaseSingleStage:
    def __init__(self, config):
        ...
        self.text_learning_rate = config.text_learning_rate
        self.af_learning_rate = config.af_learning_rate
        self.text_weight_decay = config.text_weight_decay
        self.text_train_epochs = config.text_train_epochs
        self.text_lr_min = config.text_lr_min
        self.text_warmup_lr_init = config.text_warmup_lr_init
        self.text_warmup_epochs = config.text_warmup_epochs
    ...
    def _init_creiteron(self):
        ...
        self.mselcos_creiteron = MSEL_Cos(4)
        self.mselfeat_creiteron = MSEL_Feat(4)
        self.kl_creiteron = KLDivergenceLoss_V2()


    def _init_optimizer_single_stage(self):
        params_text = []
        keys_text = []
        for key, value in self.model.named_parameters():
            if 'prompt_learner1' in key:
                lr = self.text_learning_rate
                weight_decay = self.text_weight_decay
                params_text += [{'params': [value], 'lr': lr, 'weight_decay': weight_decay}]
                keys_text += [[key]]
            if 'prompt_learner2' in key:
                lr = self.text_learning_rate
                weight_decay = self.text_weight_decay
        ...
        for key, value in self.model.named_parameters():
            if 'attention_fusion' in key:
                lr = self.af_learning_rate
                weight_decay = self.text_weight_decay
                params_text += [{'params': [value], 'lr': lr, 'weight_decay': weight_decay}]
                keys_text += [[key]]

        self.model_optimizer_text = getattr(torch.optim, 'Adam')(params_text)
        self.model_lr_scheduler_text = create_scheduler(self.model_optimizer_text,
                                                 num_epochs=self.text_train_epochs, lr_min=self.text_lr_min,
                                                 warmup_lr_init=self.text_warmup_lr_init,
                                                 warmup_t=self.text_warmup_epochs, noise_range=None)
        ...
        for key, value in self.model.named_parameters():
            ...
            if 'classifier' in key:
                lr = self.learning_rate * 2
            params_image += [{'params': [value], 'lr': lr, 'weight_decay': self.weight_decay}]
            keys_image += [[key]]

        self.model_optimizer_image = getattr(torch.optim, 'Adam')(params_image)
        self.model_lr_scheduler_image = WarmupMultiStepLR(self.model_optimizer_image, self.milestones,
                                             gamma=0.1, warmup_factor=0.01, warmup_iters=10)

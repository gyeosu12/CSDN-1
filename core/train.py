...
def train_single_stage(base, loaders, fused_text_features, config, current_epoch):
    ...
    for i, (input1_0, input1_1, input2, label1, label2) in enumerate(loader):
        ...
        with torch.no_grad():
            label_unique = torch.unique_consecutive(target)
            for j, label in enumerate(label_unique):
                fused_text_features[label] = text_features_fuse[4*j]
        ...
        text_loss = loss_i2t_text + loss_t2i_text
        af_loss = loss_i2t_af + loss_t2i_af
        image_loss = ide_loss + ide_loss_proj + (msel_loss + msel_loss_proj) + \
               config.lambda1 * (triplet_loss + triplet_loss_proj) + \
               config.lambda2 * rgb_i2t_ide_loss + config.lambda3 * ir_i2t_ide_loss + \
        ...
        base.model_optimizer_image.step()
        meter.update({
                      'loss_i2t_text': loss_i2t_text.data,
                      'loss_t2i_text': loss_t2i_text.data,
                      'loss_i2t_af': loss_i2t_af.data,
                      'loss_t2i_af': loss_t2i_af.data,
                      ...
                      'msel_loss': msel_loss.data,
                      'msel_loss_proj': msel_loss_proj.data,
                      'space_matching_loss': space_matching_loss.data,
                      'space_matching_loss_att': space_matching_loss_att.data,
                      'space_matching_loss_text': space_matching_loss_text.data,
                      })
    return meter.get_val(), meter.get_str()

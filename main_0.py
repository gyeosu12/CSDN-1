...
from core import BaseSingleStage, train_single_stage, train_stage1, train_stage2, test
...
def main(config):
    ...
    if config.mode == 'train':
        ...
        best_epoch = 0
        model._init_optimizer_single_stage()
        for current_epoch in range(start_train_epoch, config.total_train_epoch):
            ...
            model.model_lr_scheduler_text.step(current_epoch)
            model.model_lr_scheduler_image.step(current_epoch)

            _, result = train_single_stage(model, loaders, text_features, config, current_epoch)
            logger('Time: {}; Epoch: {}; LR, {}; {}'.format(time_now(), current_epoch,
                                                            model.model_lr_scheduler_image.get_lr()[0], result))
            ...
            if current_epoch + 1 >= 1 and (current_epoch + 1) % config.eval_epoch == 0:
                ...
                logger(
                    'Time: {}; Test on Dataset: {}, \nmINP: {:.4f} \nmAP: {:.4f} \n Rank-1: {:.4f} Rank-5: {:.4f} Rank-10: {:.4f} Rank-20: {:.4f}'.format(
                        time_now(),
                        config.dataset,
                        mINP,
                        mAP,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sysu_data_path', type=str, default='/home/intlab/d_drive/dataset/SYSU_MM01/')
    ...
    parser.add_argument('--text_batch-size', default=32, type=int, metavar='B', help='training batch size')
    parser.add_argument('--text_learning_rate', type=float, default=0.0003)
    parser.add_argument('--af_learning_rate', type=float, default=0.0003)
    parser.add_argument('--text_weight_decay', type=float, default=1e-4)
    parser.add_argument('--text_lr_min', type=float, default=1e-6)
    parser.add_argument('--text_warmup_lr_init', type=float, default=0.00001)
    parser.add_argument('--text_warmup_epochs', type=int, default=5)
    parser.add_argument('--text_train_epochs', type=int, default=120)
    ...
    parser.add_argument('--num_workers', default=8, type=int,
                        help='num of pos per identity in each modality')
    parser.add_argument('--output_path', type=str, default='models_0/textprompt/',
    ...
    parser.add_argument('--resume_test_model', type=int, default=115, help='-1 for no resuming')
